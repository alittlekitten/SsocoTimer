...
💡 현재 사정상 정기 패치가 11월 중순 이후로 잠시 미뤄진 상태입니다! 😄
💡 본 프로젝트 및 레포지토리와 관련한 문의사항은 issue나 개인 메일(dlscjs8646@gmail.com)을 통해서 알려주세요~!! 😄
```
...
- `React`와 `CRA`를 이용해서 간단한 스톱워치와 타이머 기능을 가진 애플리케이션을 만들어보는 연습용 레포지토리입니다.
...
- [x] 일부 이스터에그 추가
- [x] 팔레트 타입 구조 수정 (index signature 적용)
- [x] 타이틀 드래그 방지
- [x] Lap 영역을 별도로 할당하고 스크롤 부여
- [x] 다크모드일 때 확대하면 생기는 흰 영역 제거
- [x] Lap이 추가될 때 스크롤이 자동으로 맨 아래로 내려가도록 수정
...
- [x] 디데이 탭 추가
- [x] 타이틀 중 시계의 palette 색상 변경 (D-Day 탭 추가로 인한 조정)
- [x] svg 사진에 설명(alt) 추가 - svg 파일에 alt속성을 직접 줄 수 없어 우회적으로 title 적용
- [x] 탭에 들어가는 이름에 띄어쓰기 추가

### `22.06.05` - `v1.6.1`

- [x] 화면이 작은 경우 스크롤이 생기는 부분 제거
- [x] 해당 월에 없는 날짜가 화면에 남아있는 현상 제거
- [x] useDday 비즈니스 로직을 별도로 분리
- [x] 다크모드일 때 nav바에 흰색 그림자 추가
- [x] D-day부분 결과 메시지창 컬러 하이라이팅으로 가독성 향상

### `24.06.16` - `v1.6.2`

- [x] 패치 연도 변경
- [x] 시계 탭에서 아주 짧은 시간동안 0/0/0 0:0:0으로 초기화 되었다가 원래 시간으로 되돌아가는 버그를 수정
...
- 현재까지 가장 큰 4개의 기능단위인 Timer, Stopwatch, Clock, Dday에 대한 비즈니스 로직 분리를 완료했습니다.
...
- 다른 탭으로 이동해도 시간의 진행이 이루어지면 좋겠다는 피드백을 받고 시간 상태 관리 구조에 대해서 고민하게 되었습니다.
- 2개의 큰 줄기, App에서 모든 상태를 보관하고 내려주는 방식과 Store에서 시간 변수를 보관하는 방식을 고려하게 되었습니다.
- 처음에는 store의 본질적인 등장 원인인 props drilling 해결이 현재 레포지토리 구조와 같이 얕은 앱에서는 큰 메리트가 없다고 생각하게 되었습니다.
- 그래서 부모 요소(app.tsx)에 선언해놓고 자식요소로 전달하는 방식을 사용하기로 결정했고, 실제로 `v1.4.0` 패치에서 시간상태값 관련 이슈를 이 방식으로 처리하였습니다.
- `v1.5.2` 패치를 진행하면서 대규모 구조변화를 위한 렌더링 성능 테스트를 진행했습니다.
  - 실제로 store를 이용해서 필요한 상태값만 가져오는 방식으로 리렌더링이 되는 부분을 줄임으로써 매 렌더링당 약 1ms의 시간을 단축할 수 있다는 결과가 도출되었으나, 사용자의 경험에는 크게 영향을 끼치지 않을 것으로 예상되고, 당장 구조를 개선하는 것에 비해 얻는 이득이 없다고 판단되어 기존의 app에서 상태를 내려주는 구조를 유지하게 되었습니다. (Clock 컴포넌트의 상태는 상태를 유지할 필요가 없었기 때문에 별도로 분리하였습니다.)
...
- 기존에는 명시적으로 모든 속성에 대해 `string` type을 부여했었습니다.
- 이런 구조는 보다 명시적으로 타입을 보여줄 수 있지만, 새로운 속성이 추가될 때마다 타입 지정 코드를 설정해주어야 하는 문제가 있습니다.
- 어짜피 반드시 string type의 key에 string 속성만 온다면, 편리하게 Index Signature를 사용하여 한줄로 string 속성임을 명시적으로 표시할 수 있습니다.

### Lap 영역 설정

- Lap을 추가하면 화면 전체가 아래로 늘어나는 현상을 없애기 위해 별도의 영역을 Lap Container에 할당했습니다.
- 스크롤 적용까지 모두 완료했지만, 문제는 새로운 요소를 추가함에도 불구하고 기존 영역 밖에 요소가 생성되는 경우 스크롤이 맨 위에 있기 때문에 내려서 확인해야 하는 문제가 발생했습니다.
- 만약 스크롤이 생성된다면, 자동으로 맨 아래로 내려준다면 가장 최근의 Lap을 확인하기 편리할 것이라 생각했습니다.
- 처음에는 Lap state가 변경될 때(Lap 버튼을 클릭할 때) 스크롤 높이를 계산해서 맨 아래로 내리려고 했습니다.
  - 하지만 이 방법을 사용하면 setState가 scroll 계산 이후에 처리되어서 새로운 요소를 빼놓고 스크롤 높이를 계산하는 문제가 발생했습니다.
- 해당 문제를 해결하기 위해서 스크롤 계산 로직을 별도로 빼낸 후에 useEffect 안에서 스크롤 계산 로직이 돌아가도록 변경했습니다.
  - 이 방식을 통해 Lap state가 변한 이후에 스크롤의 높이를 계산해서 스크롤을 정확히 맨 아래로 내려줄 수 있었습니다.

### Open Graph 적용

- 블로그나 카카오톡 등으로 쏘코타이머 페이지를 전송할 때 별다른 텍스트 없이 링크만 전송되는 부분이 시각적으로 매력적이지 않은 부분이 있었습니다.
- 링크를 전송했을 때 이미지와 간단한 설명이 포함되도록 하기 위해서 메인 페이지에 `Open Graph`를 적용해서 사용자가 페이지를 쉽게 파악할 수 있도록 구현하였습니다.

### D-Day 기능

- 날짜를 쉽게 계산하기 위해서 D-Day 기능을 추가하였습니다.
- 날짜를 계산하는 비즈니스 로직은 `Date 객체`를 사용했습니다.
  - `getTime 함수`를 이용하여 현재 날짜에서 설정된 날짜를 빼고 나온 ms단위의 숫자값을 `나눗셈`과 `Math.floor 함수`를 통해 day단위로 분리하였습니다.
- `Date 객체` 특성상 1970년 1월 1일부터 시작하기 때문에 사용자가 얻을 수 있는 결과값을 1970년~2099년 사이의 값으로 한정하였습니다.
- 사용자가 D-Day 숫자 부분을 눈으로 쉽게 확인할 수 있도록 돕기 위해 해당 부분에 색깔을 입힐 예정에 있습니다.
...
![Honeycam 2022-04-10 05-21-00](https://user-images.githubusercontent.com/14370441/162590394-47219446-6a8b-428d-a5d5-1241c50915c6.gif)
