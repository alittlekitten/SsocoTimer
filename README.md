...
- 변수 형태로 사용하기 위한 방법으로는 emotion의 ThemeProvider, useTheme을 사용했습니다.
  - ThemeProvider는 theme 요소를 context API와 같은 방식으로 Provider 안쪽의 컴포넌트에 전달하고, useTheme을 통해서 해당 요소를 가져온 후에 props에 전달할 수 있습니다.

### 라우팅 처리

- 현재 구조에서는 라우팅이 크게 필요하지 않은 형태였지만, 개인프로젝트이기 때문에 학습용도로 작년 가을에 새롭게 업데이트된 react-router-dom v6를 사용했습니다.
